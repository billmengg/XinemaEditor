...
class TimelinePreviewCache {
  ...
  cleanupThumbnailCache() {
    ...
    this.metrics.cleanupCount++;
    console.log(
      `🧹 Thumbnail cache cleaned: removed ${toRemove.length} entries`
    );
  }
  ...
  cleanupPreviewSequenceCache() {
    ...
    this.metrics.cleanupCount++;
    console.log(
      `🧹 Preview sequence cache cleaned: removed ${toRemove.length} entries`
    );
  }
  ...
  getStats() {
    const frameCacheHitRate =
      this.metrics.frameCacheHits /
        (this.metrics.frameCacheHits + this.metrics.frameCacheMisses) || 0;

    const thumbnailCacheHitRate =
      this.metrics.thumbnailCacheHits /
        (this.metrics.thumbnailCacheHits + this.metrics.thumbnailCacheMisses) ||
      0;

    const previewSequenceHitRate =
      this.metrics.previewSequenceHits /
        (this.metrics.previewSequenceHits +
          this.metrics.previewSequenceMisses) || 0;
    ...
    return {
      ...
      memory: {
        ...
        usageMB:
          Math.round((this.metrics.memoryUsage / 1024 / 1024) * 100) / 100,
      },
      cleanup: {

...
class FrameExtractionQueue {
  ...
  async process() {
    ...
    while (
      this.queue.length > 0 &&
      this.activeExtractions < EXTRACTION_CONFIG.maxConcurrentExtractions
    ) {
...
class FrameExtractor {
  ...
  async extractFrame(character, filename, frameNumber, frameRate = 24) {
    ...
    try {
      // Check cache first
      const cachedFrame = timelinePreviewCache.getFrame(
        character,
        filename,
        frameNumber
      );
      ...
      const frameData = await this.extractFrameFromBackend(
        character,
        filename,
        frameNumber
      );
  ...
  async extractFrameFromBackend(character, filename, frameNumber) {
    const response = await fetch(
      `http://localhost:5000/api/frame-direct/${character}/${encodeURIComponent(
        filename
      )}/${frameNumber}`
    );
    ...
    if (!response.ok) {
      throw new Error(
        `Frame extraction failed: ${response.status} ${response.statusText}`
      );
    }
  ...
  async generateThumbnailSequence(clip, startTime = 0, endTime = null) {
    ...
    try {
      ...
      const cachedSequence = timelinePreviewCache.getPreviewSequence(
        clip.character,
        clip.filename
      );
      if (cachedSequence) {
      ...
      const frameInterval = Math.max(
        1,
        Math.floor(frameRate * EXTRACTION_CONFIG.frameInterval)
      );

      console.log(
        `🎬 Generating thumbnail sequence for ${clip.filename}: ${totalFrames} frames`
      );
      ...
      for (let frame = 0; frame < totalFrames; frame += frameInterval) {
        ...
        for (
          let i = 0;
          i < batchSize && frame + i < totalFrames;
          i += frameInterval
        ) {
      ...
      // Cache the sequence
      timelinePreviewCache.setPreviewSequence(
        clip.character,
        clip.filename,
        sequenceData
      );
      ...
      performanceMonitor.endTimer(timer);
      console.log(
        `✅ Generated ${thumbnails.length} thumbnails for ${clip.filename}`
      );

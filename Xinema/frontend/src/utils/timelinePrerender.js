...
class TimelinePrerender {
  ...
  async processQueue() {
    ...
    while (this.prerenderQueue.length > 0) {
      const batch = this.prerenderQueue.splice(
        0,
        this.prerenderSettings.maxConcurrent
      );
  ...
  async prerenderClipFrames({ clip, clipId, startFrame, endFrame }) {
    try {
      console.log(
        `🔄 Smart prerendering key frames for ${clipId}: ${startFrame}-${endFrame}`
      );
      ...
      console.log(
        `🎯 Generating ${keyFrames.length} key frames (every ${frameInterval} frames)`
      );
      ...
      for (let i = 0; i < keyFrames.length; i += batchSize) {
        ...
        for (let j = i; j < batchEnd; j++) {
          ...
          console.log(
            `🎬 Generating key frame ${j}: timeline=${timelineFrame}, video=${videoFrame}`
          );
          batchPromises.push(
            this.generateThumbnail(clip, videoFrame, timelineFrame)
          );
        }
        ...
        console.log(
          `📦 Key frame batch complete: ${validFrames.length} frames generated`
        );
      ...
      console.log(
        `✅ Prerender complete for ${clipId}: ${frames.length} frames`
      );
  ...
  async generateThumbnail(clip, videoFrame, timelineFrame) {
    try {
      ...
      console.error(
        `❌ Failed to generate thumbnail for frame ${videoFrame}:`,
        error
      );
      return null;
    }
  ...
  getPrerenderedFrame(clipId, timelineFrame) {
    ...
    console.log(
      `🔍 Looking for frame ${timelineFrame} in ${frames.length} prerendered frames for ${clipId}`
    );
    ...
    console.log(
      `🎯 Found closest frame: ${closestFrame.timelineFrame} (diff: ${Math.abs(
        closestFrame.timelineFrame - timelineFrame
      )})`
    );
  ...
  isClipPrerendered(clipId) {
    return (
      this.prerenderCache.has(clipId) &&
      this.prerenderCache.get(clipId).length > 0
    );
  }
  ...
  getPrerenderStatus(clipId) {
    ...
    const isComplete = this.prerenderQueue.every(
  ...
  async generateFrameOnDemand(clip, timelineFrame) {
    ...
    console.log(
      `🎬 Generating frame on-demand: timeline=${timelineFrame}, video=${videoFrame}`
    );

    try {
      const frameData = await this.generateThumbnail(
        clip,
        videoFrame,
        timelineFrame
      );
      if (frameData) {
        ...
        const frames = [
          { timelineFrame, videoFrame, imageUrl: frameData.imageUrl },
        ];
        this.updateCache(clipId, frames);
        return frameData;
      }

 ...
 */
export async function extractAudioFrame(
  clip,
  timelineFrame,
  clipStartFrames = 0
) {
  ...
  const frameStartSample = Math.floor(
    relativeFrame * FRAME_DURATION * sampleRate
  );
  const frameEndSample = Math.floor(
    (relativeFrame + 1) * FRAME_DURATION * sampleRate
  );
  ...
  const cacheEntries = Array.from(audioFrameCache.entries()).filter(([key]) =>
    key.startsWith(`${clipId}/`)
  );
